...
const App = () => {
  ...
  useEffect(() => {
    ...
    let splitTextInstance: SplitType | null = null;

    // Split the text into words
    if (titleRef.current) {
      splitTextInstance = new SplitType(titleRef.current, { types: 'words' });
    ...
    return () => {
      if (splitTextInstance) {
        ...
        // For SplitType, you may need to manually reset or clean up the HTML
        if (titleRef.current) {
          titleRef.current.innerHTML = titleRef.current.innerText; // Reset HTML
        }
      }
    };
  }, []);
